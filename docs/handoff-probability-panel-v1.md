...
> ⚠️ **先读 `docs/findings-v1.md`（2026-09-23）。**
> 本文定义的 21 特征集经完整评估后**未测出可测量的增量预测力**：单个 `d1_vol_squeeze_pct` 分位排序即达到同等判别力，且方向性预测不成立。本文的定义与参数仍然有效且被测试保护，但**不要在未读结论文档的情况下基于本文扩展特征集或重建模型**。
